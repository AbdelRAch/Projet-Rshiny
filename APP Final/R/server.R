...
server <- function(input, output, session) {
  ...
  df <- dd$df
  conso <- dd$conso
  conso_sup36 <- dd$conso_sup36
  regions_geo <- dd$regions_geo
  ...
  observeEvent(input$tab_36k, {
    ...
    output$total_energie <- renderValueBox({
      req(input$region,input$plage_puissance,input$profil)
      data <- filtered_data()
      if (input$choix_pas == "demi_horaire") {
    ...
    output$puissance_max <- renderValueBox({
      ...
      if (input$choix_pas == "demi_horaire") {
        ...
        horodate_max <- data$heure[which.max(data$total_energie_soutiree_wh)]
      } else {
  ...
  observeEvent(input$tab_prod, {
    output$main_content <- renderUI({
      tagList( 
        ...
        fluidRow(
          ...
          # selection de la plage de puissance:
          column(3,
                 selectInput("plage_puissances", "La plage de puissance", 
                             choices = unique(df$plage_de_puissance_injection), 
                             selected = unique(df$plage_de_puissance_injection)[1], 
                             multiple = TRUE)),
    ...
    observeEvent(input$fliere, {
      
      # Filtrer les valeurs de la plage de puissance selon le profil sélectionné
      filtered_plage <- df %>%
        filter(filiere_de_production == input$fliere) %>%
        pull(plage_de_puissance_injection) %>%
        unique()
      
      print(filtered_plage)
      
      # Mettre à jour les choix dans le selectInput de la plage de puissance
      updateSelectInput(session, "plage_puissances", 
                        choices = filtered_plage,
                        selected = filtered_plage[1])
    })
    ...
    filtered_data <- reactive({
      if (input$choix_pas == "demi_horaire") {
        ...
        df_filtered <- df %>%
          filter(region %in% input$region,
                 plage_de_puissance_injection %in% input$plage_puissances,
                 date %in% input$date,
    ...
    output$total_energie <- renderValueBox({
      req(input$region,input$plage_puissances,input$fliere)
      data <- filtered_data()
    ...
    output$puissance_max <- renderValueBox({
      ...
      if (input$choix_pas == "demi_horaire") {
        ...
        horodate_max <- data$heure[which.max(data$total_energie_injectee_wh)]
    ...
    output$map <- renderLeaflet({
      ...
      if (input$choix_pas == "demi_horaire") {
        ...
        df_aggregated <- df %>%
          filter(plage_de_puissance_injection %in% input$plage_puissances,
                 date %in% input$date,
                 filiere_de_production == input$fliere,
                 heure >= debut & heure <= fin)%>%
